...
class MessagesController extends Controller
{
    ...
    public function idFetchData(Request $request)
    {
        ...
        if ($request['type'] == 'user') {
            ...
            if($fetch){
                ...
                $userAvatar=Helpers::image($fetch->photo, 'user/avatar/');
            }
        }
    ...
    public function search(Request $request)
    {
        ...
        $records = User::where('id', '!=', $currentUserId)
        ->where('name', 'LIKE', "%{$input}%")
        ->whereExists(function ($query) use ($currentUserId) {
            $query->select(DB::raw(1))
                ->from('ch_messages')
                ->whereRaw('ch_messages.from_id = users.id')
                ->where('ch_messages.to_id', $currentUserId)
                ->orWhereRaw('ch_messages.to_id = users.id')
                ->where('ch_messages.from_id', $currentUserId);
        })
        ->paginate($request->per_page ?? $this->perPage);
