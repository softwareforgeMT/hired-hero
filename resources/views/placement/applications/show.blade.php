...
@section('title', 'Application Details - ' . $application->job_title)
    ...
    <div class="container">
        ...
        <div class="page-header">
            <div class="d-flex justify-content-between align-items-start gap-3">
                <div>
                    <h2 class="mb-2">{{ $application->job_title }}</h2>
                    <p class="mb-0">
                        <i class="ri-building-2-line"></i> {{ $application->company_name }}
                    </p>
                </div>
        ...
        <div class="row">
            <div class="col-lg-8">
                ...
                <div class="detail-card detail-section">
                    ...
                    <div class="detail-row">
                        ...
                        <span class="detail-value">
                            <a href="{{ $application->job_url }}" target="_blank" class="badge-link">
                ...
                <div class="detail-card detail-section">
                    ...
                    <div class="detail-row">
                        ...
                        <span class="detail-value">
                            ...
                            <small class="text-muted d-block">{{ $application->applied_at->diffForHumans() }}</small>
                        </span>
                    </div>
                    ...
                    <div class="detail-row">
                        ...
                        <span class="detail-value">
                            {{ $application->last_activity_at?->format('M d, Y') ?? '—' }}
                            @if($application->last_activity_at)
                            <small class="text-muted d-block">{{ $application->last_activity_at->diffForHumans() }}</small>
                            @endif
                        </span>
                    </div>
                </div>
                ...
                <div class="detail-card detail-section">
                    ...
                    <div class="detail-row">
                        ...
                        <span class="detail-value">
                            ...
                            <small class="text-muted d-block">in {{ $application->interview_date->diffForHumans() }}</small>
                            @elseif($application->interview_date->isPast())
                            <small class="text-muted d-block">{{ $application->interview_date->diffForHumans() }}</small>
                            @endif
                        </span>
                    </div>
                    @endif
                ...
                <!-- Offer Details -->
                @if($application->offer_date || $application->offer_salary || $application->offer_accepted !== null)
                <div class="detail-card detail-section">
                    ...
                    <div class="detail-row">
                        ...
                        <span class="detail-value">{{ $application->offer_date->format('M d, Y') }}</span>
                    </div>
                    @endif
                    ...
                    <div class="detail-row">
                        ...
                        <span class="detail-value">${{ number_format($application->offer_salary, 0) }}</span>
                    </div>
                    @endif

                    @if($application->offer_accepted !== null)
                    <div class="detail-row">
                ...
                <div class="detail-card detail-section">
                    ...
                    @if($application->used_ai_cover_letter)
                ...
                <div class="btn-group-custom">
                    <a href="{{ route('placement.applications.edit', $application) }}" class="btn btn-primary">
                    ...
                    <div class="detail-row" style="padding: 0.75rem 0;">
                        ...
                        <span class="detail-value" style="text-align: left; margin-left: 1rem;">
                            <strong style="font-size: 1.3rem; color: #ff6b35;">
                                {{ $application->applied_at->diffInDays(now()) }}
                            </strong>
                    ...
                    <div style="font-size: 0.9rem;">
                        <div style="display: flex; gap: 1rem; margin-bottom: 1.5rem;">
                            ...
                            <div>
                                ...
                                <small class="text-muted d-block">{{ $application->applied_at->format('M d, Y') }}</small>
                            </div>
                        </div>

                        @if($application->interview_date)
                        <div style="display: flex; gap: 1rem; margin-bottom: 1.5rem;">
                            <div style="color: {{ $application->interview_date->isFuture() ? '#f0ad4e' : '#0f5132' }}; font-weight: 700; min-width: 30px; text-align: center;">
                                {{ $application->interview_date->isFuture() ? '◯' : '✓' }}
                            </div>
                            <div>
                                <strong>Interview</strong>
                                <small class="text-muted d-block">{{ $application->interview_date->format('M d, Y') }}</small>
                            </div>
                        </div>
                        @endif
                        ...
                        <div style="display: flex; gap: 1rem;">
                            <div style="color: {{ $application->offer_accepted ? '#0f5132' : '#842029' }}; font-weight: 700; min-width: 30px; text-align: center;">
                                {{ $application->offer_accepted ? '✓' : '✗' }}
                            </div>
                            <div>
                                <strong>Offer {{ $application->offer_accepted ? '(Accepted)' : '(Declined)' }}</strong>
                                <small class="text-muted d-block">{{ $application->offer_date->format('M d, Y') }}</small>
                            </div>
                        </div>
                        @endif
                    </div>
