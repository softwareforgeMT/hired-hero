...
<div class="cover-letter-container">
    <div class="cover-letter-wrapper">
        ...
        <div class="cover-letter-header">
            <a href="{{ route('placement.jobs.index') }}" class="btn-back">
            ...
            <p class="header-subtitle">For {{ $jobTitle }} at {{ $companyName }}</p>
        </div>
        ...
        <div class="cover-letter-content">
            ...
            <aside class="cover-letter-sidebar">
                ...
                <div class="detail-card">
                    ...
                    <div class="detail-item">
                        ...
                        <p class="job-description-preview">{{ Str::limit($jobDescription, 200) }}</p>
                    </div>
                    @endif
                </div>
                ...
                <div class="detail-card usage-card">
                    ...
                    <a href="{{ route('front.pricing') }}" class="btn btn-primary w-100 mt-2">
                        Upgrade Plan
                    </a>
                    @else
                    <div class="usage-bar-container">
                        @if ($coverLetterLimit === 'unlimited')
                        <div class="usage-info">
                            <span class="usage-label">Covers Generated</span>
                            <span class="usage-value">{{ $coversUsed }} <span class="usage-unlimited">∞</span></span>
                        </div>
                        ...
                        <div class="usage-info">
                            ...
                            <span class="usage-value">{{ $coversUsed }} / {{ $coverLetterLimit }}</span>
                        </div>
                        <div class="usage-bar">
                            <div class="usage-fill" style="width: {{ ($coversUsed / $coverLetterLimit) * 100 }}%;"></div>
                        </div>
                        <p class="usage-text">{{ $coversRemaining }} cover{{ $coversRemaining !== 1 ? 's' : '' }} remaining</p>
                        @endif
                    </div>
                    @endif
                </div>
            ...
            <main class="cover-letter-main">
                ...
                <div class="cover-letter-actions">
                    ...
                    <a href="{{ route('placement.jobs.index') }}" class="btn btn-primary">
                        Back to Job Matches
                    </a>
                    @if (!$hasExceededLimit)
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0">
            ...
            <div class="modal-footer border-0">
                ...
                <a href="{{ route('front.pricing') }}" class="btn btn-warning">
                    View Plans
                </a>
            </div>
        </div>
...
<script>
    ...
    const hasExceededLimit = {{ $hasExceededLimit ? 'true' : 'false' }};
    ...
    function generateCoverLetter() {
        ...
        fetch('{{ route("placement.covers.store") }}', {
                method: 'POST',
                headers: {
    ...
    function finalizeCoverLetter() {
        ...
        fetch('{{ route("placement.covers.finalize") }}', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.getAttribute('content'),
