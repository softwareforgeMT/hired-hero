...
<div class="wizard-step-content">
    ...
    <div class="skills-modal-overlay" id="skillsModal">
        <div class="skills-modal-content">
            ...
            <p class="modal-subtitle">{{ count($extractedSkills) }} skills extracted from your resume</p>
    ...
    <div class="form-content" id="formContent" style="display: {{ $hasSkills ? 'none' : 'block' }};">
        <h2 class="step-title mb-2">Your Skills Profile</h2>
        <p class="step-description mb-4">
            @if($hasSkills)
                {{ count($extractedSkills) }} skills extracted. Add more or remove any you don't need.
            @else
                Add your key skills to improve job matching.
            @endif
        </p>

        <form action="{{ route('placement.wizard.submit', ['step' => 7]) }}" method="POST">
            @csrf
            ...
            <div class="form-group mb-4">
                ...
                <div class="skills-container">
                    @foreach($extractedSkills as $skill)
                    <div class="skill-tag extracted-skill" data-skill="{{ $skill }}">
                        <span>{{ $skill }}</span>
                        <button type="button" class="skill-remove" onclick="skillsManager.removeSkill('{{ $skill }}', 'extracted')">✕</button>
                    </div>
                    @endforeach
                </div>
            </div>
            ...
            <div class="form-group mb-4">
                <label for="manual_skills" class="form-label fw-semibold mb-2">
                    @if($hasSkills) Add More Skills @else Add Skills @endif
                </label>
            ...
            <div class="d-flex gap-2 justify-content-between align-items-center pt-3 border-top">
                ...
                <div>
                    <a href="{{ route('placement.wizard.step', ['step' => 6]) }}" class="btn btn-sm btn-outline-secondary">Back</a>
